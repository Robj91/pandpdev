...
{ 
 ...
 sp_touch_damage(&current_sprite, 0);
   ...
   //"push" takes priority over "push_custom"
   if (&save_x == 0)
   {
    &save_x = sp_custom("push_custom", &current_sprite, -1);
    if (&save_x == 0)
    {
     &save_x = sp_custom("push_custom", 1, -1);
    }
    if (&save_x > 1)
    {
     sp_custom("pushdelay", &current_sprite, &save_x);
   ...
   //start the timer before push can activate
    ...
    sp_custom("PPtimer_delay", &current_sprite, 0);
    
    &save_x = sp_custom("pushdelay", &current_sprite, -1);
    if (&save_x == 0)
    {
     &save_x = sp_custom("pushdelay", 1, -1);
    }
    if (&save_x > 0)
    {
     sp_custom("PPtimer_delay", &current_sprite, &save_x);
