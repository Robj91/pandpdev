//allows push to happen after all initial checks and setup is done.
//Constantly moves the sprite, locks dinks position relative to object he's pushing and detects when dink stops pushing the object

void main(void)
{ 
 //retrieve the current sprite being moved.
 int &hupsprite = &save_x;

 //store the player map for later, in case the player changes screens
 &save_x = &player_map; 
 sp_custom("screen", &hupsprite, &save_x);
 
 //reset the custom key which is used to tell this script to kill itself.
 sp_custom("hupkill", &hupsprite, 0);

 //update the "MovePosNeg" custom key
 &save_x = sp_custom("PushPosNeg", &hupsprite, -1);
 sp_custom("MovePosNeg", &hupsprite, &save_x);
 ...
 int &mco;
    ...
    if (&save_x > 0)
    {
     ...
     else
     {
      ...
      if (&save_y == &save_x)
      {
       ...
       &save_y = sp_custom("pushscript", &mco, -1);
       if (&save_y <= 0)
       {
        &save_x += 1;
        goto multipushKILL;
       ...
       {   
        ...
        &save_x = sp_custom("PhisSpriteClone", &hupsprite, -1);
        sp_active(&save_x, 0);       
  ...
  &save_x = sp_custom("Move-Nohard", &hupsprite, -1);
  if (&save_x > 0)
  {
   &save_x = sp_custom("Dink-Push-Limit", &hupsprite, -1);
  }
  else
  {
   &save_x = sp_custom("hupmove", &hupsprite, -1); 
  }

   //save the limit we'll be using in a new custom key so we only have to run this check once.
   sp_custom("huplimit", &hupsprite, &save_x);
...
loop: 
 ...
 if (&hupsprite > 0)
 {
  ...
  wait(1); 
   ...
   &save_x = sp_custom("PosAltered", &hupsprite, -1);
   if (&save_x <= 0)
   {
    &save_x = sp_custom("spriteox", &hupsprite, -1);
    &save_y = sp_x(&hupsprite, -1);
    if (&save_x != &save_y)
     sp_custom("PosAltered", &hupsprite, 1);
     
    &save_x = sp_custom("spriteoy", &hupsprite, -1);
    &save_y = sp_y(&hupsprite, -1);
    if (&save_x != &save_y)
     sp_custom("PosAltered", &hupsprite, 1);
   }

   //check if this sprite has already ran a phisend instance
   &save_x = sp_custom("Initiate-END", &hupsprite, -1);
   if (&save_x > 0)
   {
    //kill the sprite clone
    &save_x = sp_custom("PhisSpriteClone", &hupsprite, -1);
    ...
    &save_x = sp_custom("pushscript", &hupsprite, -1);
    if (&save_x > 0)
   ...
   &save_x = &player_map;
   &save_y = sp_custom("screen", &hupsprite, -1);
   if (&save_x != &save_y)
   {
    //we can just end it here - don't have to remotely kill off phispush.c
    //because it is not attached to 1000, so it'll die on screen change anyway.
    external("PhisEnd", "end", 2, 0, 0, &hupsprite);
    kill_this_task();
   }
  ...
  &save_x = sp_custom("inventory", &hupsprite, -1);
  if (&save_x > 0)
  {
   //He has..
    external("DinkSpeed", "speedlock", &hupsprite);
    
    &save_x = sp_speed(&hupsprite, -1);
   ...
   //make clone sprite's speed match &hupsprite
   &save_y = sp_custom("PhisSpriteClone", &hupsprite, -1);
   sp_speed(&save_y, &save_x);

   //assure dink's speed is set to -1, frame delay is correct for push, and sprite speed is correct.
    set_dink_speed(-1);

   //reset inventory custom key.
   sp_custom("inventory", &hupsprite, 0);
  } 
  	   ...
  	   /////////////////////////////////////////
           ...
           &mco = sp_custom("move-status", &hupsprite, -1);
           &save_y = 0;
           if (&mco > 0)
           {
            ...
            if (&mco == 1)
            {
             ...
             &save_x = sp_custom("Idle-SkipChecks", &hupsprite, -1);
             if (&save_x == 1)
              &save_y = 1;
            }
            if (&save_y == 0)
            {
             &mco = sp_custom("huplimit", &hupsprite, -1);
             //check for a override of move limit by dmod developer
             &save_x = sp_custom("Enable-Limit", &hupsprite, -1);
             if (&save_x > 0)
             {
              ...
              &save_x = sp_custom("move-axis", &hupsprite, -1);
              if (&save_x == 1)
              {
               &save_x = sp_custom("PushPosNeg", &hupsprite, -1);
               if (&save_x == 1)
                &save_y = sp_custom("setXmin", &hupsprite, -1);
               else
                &save_y = sp_custom("setXmax", &hupsprite, -1);
              }
              else
              {
               &save_x = sp_custom("PushPosNeg", &hupsprite, -1);
               if (&save_x == 1)
                &save_y = sp_custom("setYmin", &hupsprite, -1);
               else
                &save_y = sp_custom("setYmax", &hupsprite, -1);
              }
             ...
             &save_x = sp_custom("PhisSpriteClone", &hupsprite, -1);
             &mco = sp_custom("move-axis", &hupsprite, -1);
             if (&mco == 1)
              &save_x = sp_x(&save_x, -1);
             else
              &save_x = sp_y(&save_x, -1);
              
             &mco = sp_custom("PhisSpriteClone", &hupsprite, -1);
             if (sp_custom("hupseq", &hupsprite, -1) == 312)
             {
              if (&save_x >= &save_y) 
               sp_y(&mco, &save_y);
             } 
             if (sp_custom("hupseq", &hupsprite, -1) == 314) 
             { 
              if (&save_x <= &save_y) 
               sp_x(&mco, &save_y);
             } 
             if (sp_custom("hupseq", &hupsprite, -1) == 316) 
             {
              if (&save_x >= &save_y)  
               sp_x(&mco, &save_y);
             }   
             if (sp_custom("hupseq", &hupsprite, -1) == 318) 
             {
              if (&save_x <= &save_y)
               sp_y(&mco, &save_y);
             }
             ...
             &save_x = sp_x(&mco, -1);
             &save_y = sp_y(&mco, -1);
             sp_x(&hupsprite, &save_x);
             sp_y(&hupsprite, &save_y);
            }
           }
  	   /////////////////////////////
      ...
      //////////////////////////////////////
      //don't need to run this the first time around. Skipping saves lag in older engines.
      &save_x = sp_custom("move-status", &hupsprite, -1);
      &save_y = 0;
      if (&save_x > 0)
      {
       //also don't run this if the sprite hasn't moved since last limit check..
       ...
       if (&save_y == 0)
       {
        //Relock Dink relative to object - PUSH
        &save_x = sp_custom("hupdiff", &hupsprite, -1);
        if (&save_x == -1111)
         &save_x = -1;
  	...
  	if (&save_x == 1)
  	{
   	 &save_x = sp_custom("Idle-SkipChecks", &hupsprite, -1);
   	 if (&save_x == 0)  
    	  sp_custom("Idle-SkipChecks", &hupsprite, 1); 
  	}

  //run the MoveDetectDuring proc of the current sprite
  &save_x = is_script_attached(&hupsprite);
  if (&save_x > 0)
  {
   run_script_by_number(&save_x, "MoveDetectDuring");
  }

   //check to see if the player has moved, if so his seq will change and we'll know dink has stopped pushing
   &save_x = sp_custom("hupseq", &hupsprite, -1);
   &save_y = sp_pseq(1, -1);
   if (&save_y != &save_x)
    goto hupkill;

   //a manual termination has occured and ended push, kill off this script.
   &save_x = sp_custom("hupkill", &hupsprite, -1);
   if (&save_x > 0)
   {
  ...
  &save_x = sp_custom("stopmove", &hupsprite, -1);
  if (&save_x > 0)
  {
   &mco = 0;
   goto limit;
     	...
     	///////////////////////////////
           ...
           &save_x = sp_custom("move-axis", &hupsprite, -1);
           if (&save_x == 1)
            &save_x = sp_x(&hupsprite, -1);
           else
            &save_x = sp_y(&hupsprite, -1);
            
           sp_custom("limitreached", &hupsprite, 0);
           if (sp_custom("hupseq", &hupsprite, -1) == 312)
           {
            if (&save_x >= &save_y) 
             sp_custom("limitreached", &hupsprite, 1);
           } 
           if (sp_custom("hupseq", &hupsprite, -1) == 314) 
           { 
            if (&save_x <= &save_y) 
  ...
  //make sure the push seq is still correct and hasn't changed.
  &save_x = sp_custom("hupseq", &hupsprite, -1);   
  &save_y = sp_pseq(1, -1); 
  if (&save_x == &save_y)
  {
   ...
   &save_x = sp_custom("limitreached", &hupsprite, -1);
   if (&save_x > 0)
    goto limit;
   ...
   if (&save_x != 2)
   {
    ...
    &save_x = sp_custom("huplimit", &hupsprite, -1);
    &save_y = sp_custom("PhisSpriteClone", &hupsprite, -1);
    if (sp_custom("hupseq", &hupsprite, -1) == 312)
   ...
   //set the move-status
   sp_custom("move-status", &hupsprite, 2);
...
limit:  
  ...
  if (&mco == 0)
  {
   ...
   //a move limit was reached - stop the sprite
   if (sp_custom("hupseq", &hupsprite, -1) == 316) 
    move(&save_y, 6, -200, 1);
   ...
   if (&save_x > 1)
   {
    ...
    sp_custom("last-status", &hupsprite, &save_x);
    //let script know limit checks and relock need to be performed.
    sp_custom("Idle-SkipChecks", &hupsprite, 0);
   }
    
   sp_custom("move-status", &hupsprite, 1);
   ...
   if (&save_y >= 1)
   {
    ...
    sp_frame(1, 0);
    if (&save_y == 2)
     external("dsmove", "limit", &hupsprite);
   }
   else
   {
  ...
  else
  {
   ...
   &save_x = sp_custom("last-status", &hupsprite, -1);
   &save_y = sp_custom("move-status", &hupsprite, -1);
   if (&save_y > 1)
   {
    if (&save_x != &save_y)
     sp_custom("last-status", &hupsprite, &save_y);
   }
  }
