void pull(void)
{
//PULL - gets all the stuff ready in preperation for pull, and spawns pup.c

 //a custom key to mark this proc has proceeded
 sp_custom("Initiate-Pull", &current_sprite, 1);

 //pull activation fix. If this proc has been called, "Pbutton" custom key's value is at least equal to 1.
 //set it to 2, so phisbo.c knows that it should defeinitely reset it once the proc is over.
 //it fixes a single-press of the key being detected as a hold of the key in phisbo.c
 sp_custom("Pbutton", &current_sprite, 2);
 ...
 ///////////////////////////////////////
  ...
  if (&val1 == 0)
  {
   ...
   phisdirfix: 
    ...
    //assure we are not trying to save a value of -1 in a custom key.
    if (&val1 == -1)
     &val1 = -1111;
...
dirphiscalccont:
 ...
 sp_custom("inventory", &current_sprite, 0);
    ...
    //get the limit of how far the object can be pulled on the screen.
    external("PullLim", "plimit");
    
    //check the outcome of the above called procedure and if we should continue or end push. 		
    &save_x = &return;
    if (&save_x > 0)
    {
     external("PhisEnd", "end", 3, 2, 0, &current_sprite);
     kill_this_task();
    }
 ...
 //this is to store the fact that all checks have passed and pull is active.
 sp_custom("pullactive", &current_sprite, 1);   
 ...
 //Here, I'll be loading Dink's regular diagonal walk sequence graphics (sequence 70) into the push diags
 //push diags are usually blank (nothing loaded), but this is required
 //so that there is a sequence there, so we can detect IF Dink's seq changes and he is trying to disengage
 //basically, allows to cancel pull by walking in any diagonal direction.
 ...
 //change base idle, base walk, and make dinks push play in reverse to give pull effect
 sp_base_idle(1, 520);
 sp_base_walk(1, 310);
 sp_reverse(1, 1); 
 ...
 sp_custom("pullscript", &current_sprite, &save_x);
   ...
   sp_custom("PullSpriteClone", &current_sprite, &save_x);
 ...
 //Then retrieve it STRAIGHT AWAY in "pup.c" before it can have a chance to change. 
 &save_x = &current_sprite;
 
 //Spawn Pup.c - pup.c does a few different things, see the comments in that script for more info
 spawn("pup");


 //loop to check when player releases the 'z' key. Holding a key causes the dink engine to call it's script every wait(199)
 //So.. the Pull key script (key-90.c) pokes the sprites 'pull' proc, which increments the 'Pbutton' custom key by 1 every wait(199)
 //we store Pbutton, wait 200 and check if it's changed, if it has, the player is still holding the Pull key.
releaseloop: 
 &val1 = sp_custom("Pbutton", &current_sprite, -1);
 wait(200);
 &save_x = sp_custom("Pbutton", &current_sprite, -1);
 if (&val1 != &save_x) goto releaseloop;

 //Z key released, let's make pup.c stop what it's doing and kill itself
  sp_custom("pupkill", &current_sprite, 1);
...
void multipullend(void)
{
 external("PhisEnd", "end", 6, 0, 0, &current_sprite);
 kill_this_task();
}
