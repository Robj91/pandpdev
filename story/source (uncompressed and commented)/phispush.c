void phisbo(void)
{
//PUSH - gets all the stuff ready in preperation for push, and spawns hup.c
 
 //a custom key to mark this proc has proceeded
 sp_custom("Initiate-Push", &current_sprite, 1);
...
dirphiscalccont:
 ...
 if (&val1 == 0) 
 {
  ...
  if (&save_x > 0)
  {
   sp_custom("pushbypass", &current_sprite, 1);
   kill_this_task();
  }
 }
 ...
 sp_custom("inventory", &current_sprite, 0);
  ...
  //get the limit of how far the object can be pushed on the screen.
  external("PhisLim", "phislimit"); 
 
  //check the outcome of the above called procedure and if we should continue or end push.
  &save_x = &return;
  if (&save_x > 0)
  {
   external("PhisEnd", "end", 4, 2, 0, &current_sprite);
   kill_this_task();
  }
   ...
   //FAIL SAFE CHECKS//
   ////////////////////
   //make sure pullkey isn't held down - if so, pull takes priority.
   //Requires a wait to give pullkey detection.
   wait(0);
   &save_x = sp_custom("pullkeyheld", &current_sprite, -1);
   if (&save_x > 0)
   {
    sp_custom("pushbypass", &current_sprite, 1);
    external("PhisEnd", "end", 4, 3, 0, &current_sprite);
    kill_this_task();
   }
 ...
 //this is to store the fact that all checks have passed and push is active.
 sp_custom("pushactive", &current_sprite, 1); 
 ...
 //so we can detect IF Dink's seq changes and he is trying to disengage
 //basically, allows to cancel push by walking in any diagonal direction.
 ...
 //change dinks base walk to the push sequence
 sp_base_walk(1, 310);
 
 //save the current script in custom key "pushscript"
 &save_x = &current_script;
 sp_custom("pushscript", &current_sprite, &save_x);
   ...
   sp_custom("PhisSpriteClone", &current_sprite, &save_x);
 ...
 //Then retrieve it STRAIGHT AWAY in "hup.c" before it can have a chance to change.
 &save_x = &current_sprite; 
 
 //Spawn hup.c - hup.c does a few different things, see the comments in that script for more info
 spawn("hup"); 

 //loop forever until hup.c pokes this script and ends it. Long ass wait time so this loop doesn't add to lag.
...
{
 external("PhisEnd", "end", 5, 0, 0, &current_sprite);
 kill_this_task();
}
