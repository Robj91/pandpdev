...
{
 //check if we should say x or y position.
 &save_x = sp_custom("XorY", &current_sprite, -1);
 if (&save_x == 1)
 {
  //y poisiton
  &save_x = sp_y(&current_sprite, -1);
  say("`%My current Y position is &save_x", &current_sprite);
 }
 else
 { 
  //x position
  &save_x = sp_x(&current_sprite, -1);
  say("`%My current X position is &save_x", &current_sprite);  
 }
 goto stopex;
}
...
void hit(void)
{
 ...
 &save_x = sp_custom("XorY", &current_sprite, -1);
 if (&save_x == 0)
  sp_custom("XorY", &current_sprite, 1);
 else
  sp_custom("XorY", &current_sprite, 0);
